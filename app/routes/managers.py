...
def assign_crew(flight_id):
    ...
    if request.method == 'POST':
        ...
        fa_ids = request.form.getlist('flight_attendant_id')
        ...
        try:
            with get_db_cursor(commit=True) as cursor:
                ...
                cursor.execute("DELETE FROM FlightPilotAssignment WHERE flight_id = %s", (flight_id,))
                cursor.execute("DELETE FROM FlightAttendantAssignment WHERE flight_id = %s", (flight_id,))
                
                # Insert pilot assignments
                pilot_insert = "INSERT INTO FlightPilotAssignment (pilot_id, flight_id) VALUES (%s, %s)"
                for pilot_id in pilot_ids:
                    cursor.execute(pilot_insert, (pilot_id, flight_id))
                ...
                for fa_id in fa_ids:
                    cursor.execute(fa_insert, (fa_id, flight_id))
            ...
            with get_db_cursor(commit=True) as cursor:
                ...
                for row in range(1, rows_count + 1):
                    for col in cols:
                        ...
                        cursor.execute(seat_insert, (plane_id, class_type, seat_number))
